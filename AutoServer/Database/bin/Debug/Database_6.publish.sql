/*
Скрипт развертывания для Database

Этот код был создан программным средством.
Изменения, внесенные в этот файл, могут привести к неверному выполнению кода и будут потеряны
в случае его повторного формирования.
*/

GO
SET ANSI_NULLS, ANSI_PADDING, ANSI_WARNINGS, ARITHABORT, CONCAT_NULL_YIELDS_NULL, QUOTED_IDENTIFIER ON;

SET NUMERIC_ROUNDABORT OFF;


GO
:setvar DatabaseName "Database"
:setvar DefaultFilePrefix "Database"
:setvar DefaultDataPath "C:\Program Files\Microsoft SQL Server\MSSQL11.SQLEX\MSSQL\DATA\"
:setvar DefaultLogPath "C:\Program Files\Microsoft SQL Server\MSSQL11.SQLEX\MSSQL\DATA\"

GO
:on error exit
GO
/*
Проверьте режим SQLCMD и отключите выполнение скрипта, если режим SQLCMD не поддерживается.
Чтобы повторно включить скрипт после включения режима SQLCMD выполните следующую инструкцию:
SET NOEXEC OFF; 
*/
:setvar __IsSqlCmdEnabled "True"
GO
IF N'$(__IsSqlCmdEnabled)' NOT LIKE N'True'
    BEGIN
        PRINT N'Для успешного выполнения этого скрипта должен быть включен режим SQLCMD.';
        SET NOEXEC ON;
    END


GO
USE [$(DatabaseName)];


GO
PRINT N'Выполняется удаление [modules].[Levels].[idx_Levels_ClientId]...';


GO
DROP INDEX [idx_Levels_ClientId]
    ON [modules].[Levels];


GO
PRINT N'Выполняется удаление [modules].[Levels].[idx_Levels_CreationDate]...';


GO
DROP INDEX [idx_Levels_CreationDate]
    ON [modules].[Levels];


GO
PRINT N'Выполняется создание [modules].[Levels].[idx_Levels_ClientId]...';


GO
CREATE NONCLUSTERED INDEX [idx_Levels_ClientId]
    ON [modules].[Levels]([ClientId] ASC);


GO
PRINT N'Выполняется создание [modules].[Levels].[idx_Levels_CreationDate]...';


GO
CREATE NONCLUSTERED INDEX [idx_Levels_CreationDate]
    ON [modules].[Levels]([CreationDate] ASC);


GO
PRINT N'Обновление завершено.'
GO
